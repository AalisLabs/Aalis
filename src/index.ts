...
async function main() {
  ...
  // ── 子命令分发（通用）─────────────────────────────────
  // `aalis <name> [args...]` 等价于在聊天里发送 `/<name> args...`：
  // 查询 commands 服务，命中则执行返回字符串并干净退出；不命中则按
  // 正常守护进程模式继续启动。
  // 该分发**与具体命令解耦** —— doctor / migrate / etc. 各自注册命令即可。
  const subArgv = process.argv.slice(2);
  if (subArgv.length > 0) {
    const exitCode = await tryDispatchSubcommand(app, subArgv);
    if (exitCode !== null) {
      await app.stop();
