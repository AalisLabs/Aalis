...
// 一处就多一个可能漏抄约定的竞态源。
//
// 拆卸一律走 retireBatch（plugin-activation.ts），不许手写四步。
// plugin.ts 对 entry.activation 零直接写入——清引用只走 retireBatch。
// enable 把 disabled/error 拉回 pending 是点名的例外。新增写入点必须：
//   1. 过刀单（core 修改逐条拍板）；
//   2. 对照 retireBatch JSDoc 的顺序约定自查；
...
describe('插件状态机写入点定格', () => {
  for (const [file, expected] of Object.entries(FROZEN)) {
    it(`${file}：state=${expected.state} / activation=${expected.activation}`, () => {
      ...
      expect(
        counts,
        `${file} 的 entry.state/entry.activation 写入点数变了。若是新增拆卸路径：改用 retireBatch，` +
  ...
  it('orchestration/plugin.ts 对 entry.activation 零直接写入', () => {
    ...
    expect(writes, '拆卸清引用只走 retireBatch，plugin.ts 不得直接写 entry.activation').toHaveLength(0);
  });
