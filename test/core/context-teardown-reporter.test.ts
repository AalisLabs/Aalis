...
describe('Activation 拆卸路径：宿主 logger 抛错不中断拆卸', () => {
  it('初始化等待超时的上报抛错：disposeAsync 仍完成，清理链照跑', async () => {
    const app = mkApp('等待初始化落定');
    const child = activationHost(app).create(rootActivation(app), 'slow');
    child.resources.lifecycle.trackInitialization(new Promise(() => {}));
    let cleaned = false;
    child.resources.onDispose(() => {
      cleaned = true;
    });
