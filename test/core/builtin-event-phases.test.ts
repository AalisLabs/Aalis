...
import { App, definePlugin, events, lifecycle } from '../../packages/core/src/index.js';
...
describe('内置事件：通知节不等监听器', () => {
  it('plugin:loaded 的慢监听器不挡住后续插件的激活', async () => {
    ...
    bus.on('plugin:loaded', async name => {
    ...
    void app.plugins.register(definePlugin({ name: 'first', apply: () => void applied.push('first') }));
    await app.plugins.register(definePlugin({ name: 'second', apply: () => void applied.push('second') }));
    expect(await settlesWithin(app.plugins.idle(), 200), 'flight 在等 plugin:loaded 的监听器').toBe(true);
    expect(applied).toEqual(['first', 'second']);
    expect(app.plugins.getPlugin('first')?.state).toBe('active');
    expect(app.plugins.getPlugin('second')?.state).toBe('active');
  ...
  it('plugin:loaded 监听器里 await plugins.idle() 不死锁', async () => {
    ...
    await app.plugins.register(definePlugin({ name: 'p', apply() {} }));
...
describe('内置事件：屏障节等监听器', () => {
  it('start() 的三个屏障按序推进，每一步都等上一步的监听器全部完成', async () => {
    ...
    bus.on('app:starting', slow('starting'));
    bus.on('app:ready', slow('ready'));
    bus.on('app:started', () => void order.push('started'));
  ...
  it('app:restarting 的监听器全部完成后才把控制交给 RestartStrategy', async () => {
    const order: string[] = [];
    let handed!: () => void;
    ...
    const { events: bus } = app.bind({ events });
    bus.on('app:restarting', async () => {
      await new Promise<void>(r => setTimeout(r, 15));
      order.push('restarting');
    });
