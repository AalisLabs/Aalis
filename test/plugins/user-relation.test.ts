...
describe('plugin-user-relation: getCommunityOverview SLPA 原生重叠社区', () => {
  it('双团 + 桥梁人物：SLPA 应给出桥梁人物的多社群隶属（memberships.length>=2 或 bridges 非空）', async () => {
    const { service } = await makeService();
    // 构造两个 4 人团 + 1 个桥梁人物同时参与两团事件
    const groupAIds: string[] = [];
    for (let i = 0; i < 4; i++) {
      const p = await service.observePerson('onebot', `A-${i}`, `A${i}`);
      groupAIds.push(p.id);
    }
    const groupBIds: string[] = [];
    for (let i = 0; i < 4; i++) {
      const p = await service.observePerson('onebot', `B-${i}`, `B${i}`);
      groupBIds.push(p.id);
    }
    const bridge = await service.observePerson('onebot', 'bridge', 'Bridge');
    // A 团事件：A0~A3 + bridge 同时参与
    const eA = await service.createEvent({ title: 'EventA', evidence: [ev()] });
    for (const pid of [...groupAIds, bridge.id]) {
      await service.addPersonEventEdge({
        fromPersonId: pid,
        toEventId: eA.id,
        role: 'participant',
    ...
    // B 团事件：B0~B3 + bridge 同时参与
    const eB = await service.createEvent({ title: 'EventB', evidence: [ev()] });
    for (const pid of [...groupBIds, bridge.id]) {
      await service.addPersonEventEdge({
        fromPersonId: pid,
        toEventId: eB.id,
        role: 'participant',
    ...
    const r = await service.getCommunityOverview({ algorithm: 'slpa' });
    expect(r.algorithm).toBe('slpa');
    expect(r.numCommunities).toBeGreaterThanOrEqual(2);
    // bridges 必须能识别到桥梁人物并给出多社群权重分布
    const bridgeRow = r.bridges.find(b => b.id === bridge.id);
    expect(bridgeRow).toBeTruthy();
    expect(bridgeRow!.communityWeights.length).toBeGreaterThanOrEqual(1);
    // bridge 的总 cross-weight 应该明显高于团内成员
    const aMemberRow = r.bridges.find(b => groupAIds.includes(b.id));
    if (aMemberRow) {
      const bridgeSum = bridgeRow!.communityWeights.reduce((s, w) => s + w.weight, 0);
      const aSum = aMemberRow.communityWeights.reduce((s, w) => s + w.weight, 0);
      expect(bridgeSum).toBeGreaterThanOrEqual(aSum);
    }
  });

  it('Louvain 永远单元素 memberships，bridges 输出 communityMemberships 长度为 1', async () => {
    const { service } = await makeService();
    // 两团 + 跨团弱桥
    for (let g = 0; g < 2; g++) {
      const ids: string[] = [];
      for (let i = 0; i < 4; i++) {
        const p = await service.observePerson('onebot', `lv-g${g}-${i}`, `LvG${g}P${i}`);
        ids.push(p.id);
      }
      const inner = await service.createEvent({ title: `LvG${g}Ev`, evidence: [ev()] });
      for (const pid of ids) {
        await service.addPersonEventEdge({
          fromPersonId: pid,
          toEventId: inner.id,
    ...
    const r = await service.getCommunityOverview({ algorithm: 'louvain' });
    expect(r.algorithm).toBe('louvain');
    for (const b of r.bridges) {
      expect(b.communityMemberships.length).toBe(1);
      expect(b.communityMemberships[0].weight).toBe(1);
      expect(b.communityMemberships[0].id).toBe(b.communityId);
    }
  });

  it('SLPA 确定性：同一快照连续两次调用结果一致（节点 id 字典序导出种子）', async () => {
    const { service } = await makeService();
    for (let i = 0; i < 6; i++) {
      const p = await service.observePerson('onebot', `det-${i}`, `Det${i}`);
      const e = await service.createEvent({ title: `DEv${i}`, evidence: [ev()] });
      await service.addPersonEventEdge({
        fromPersonId: p.id,
        toEventId: e.id,
        role: 'participant',
    ...
    const r1 = await service.getCommunityOverview({ algorithm: 'slpa' });
    const r2 = await service.getCommunityOverview({ algorithm: 'slpa' });
