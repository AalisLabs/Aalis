...
// 驱动面全部走公开面：宿主提供假 storage + skills 服务 + 组装器直驱 +
// hooks.run('agent:input:before') 驱动 triggers 自动激活。
// ════════════════════════════════════════════════════════════
...
async function setup(config: Record<string, unknown> = {}) {
  ...
  await app.plugins.register(skillsPlugin, config);
  await app.plugins.idle();
  const service = host.services.get(skills);
  if (!service) throw new Error('skills 服务未注册');
  /** 组装器只要两样能力：枚举 agent:prompt 的交付、记日志 */
  const assembly: PromptAssemblyCaps = { contributions: host.contributions, logger: host.logger };
  return { app, store, skills: service, assembly, hooks: host.hooks };
}

/**
 * 锚位探针插件：按给定局部 id 与锚位各交一份贡献，用相对次序钉死被测块落在哪个槽。
 * 探针的身份就是插件实例 id（全局键的前半），故排序要求直接写进插件名里。
 */
function anchorProbe(name: string, blocks: Array<{ id: string; anchor: PromptAnchor; text: string }>) {
  return definePlugin({
    name,
    uses: { contributions },
    apply({ contributions }) {
      for (const b of blocks) {
        contributions.contribute('agent:prompt', { id: b.id, anchor: b.anchor, build: () => b.text });
      }
    },
  });
}
...
describe('plugin-skills: agent:prompt 贡献', () => {
  it('discoveryEnabled=true 且有技能：discovery 块是静态路标，不列技能名', async () => {
    const { app, skills, assembly } = await setup();
    ...
    await app.plugins.register(
      anchorProbe('probe', [
        { id: 'idn', anchor: 'identity', text: 'IDN' },
        { id: 'cx', anchor: 'context', text: 'CX' },
      ]),
    );
    await app.plugins.idle();
    ...
    await assemblePromptContributions(assembly, { messages, sessionId: 's-1' });
  ...
  it('rescan 发现外部放入的技能后，discovery 路标才出现', async () => {
    const { store, skills, assembly } = await setup();
    const messages = baseMessages();
    await assemblePromptContributions(assembly, { messages, sessionId: 's-1' });
    expect(messages.some(isDiscovery)).toBe(false);

    store.seed('data:/skills/gamma/SKILL.md', '---\nname: gamma\ndescription: 外部放入的技能\n---\n\nGAMMA-BODY\n');
    await skills.rescan();
  ...
  it('loadSkillForSession 后组装：激活块含 SKILL 正文，且只在该 sessionId 出现', async () => {
    ...
    // 锚位探针：identity 给上界，context 给下界。两个探针的插件名必须把被测插件的全局键
    // （`@aalis/plugin-skills/…`）**夹在中间**——同槽内按全局键码元序排布，探针若落在
    // 被测块同侧，错标锚位后次序不变、断言恒真。'@' 的码元在数字之后、字母之前，
    // 故 context 探针取数字开头（排在插件键之前）、identity 探针取 zz-（排在之后）。
    await app.plugins.register(anchorProbe('zz-probe-identity', [{ id: 'idn', anchor: 'identity', text: 'IDN' }]));
    await app.plugins.register(anchorProbe('0-probe-context', [{ id: 'cx', anchor: 'context', text: 'CX' }]));
    await app.plugins.idle();

    const loaded = baseMessages();
    await assemblePromptContributions(assembly, { messages: loaded, sessionId: 's-1' });
    ...
    await assemblePromptContributions(assembly, { messages: other, sessionId: 's-2' });
  ...
  it('未知技能名 loadSkillForSession 返回 false，且不产生激活块', async () => {
    const { skills, assembly } = await setup();
    await skills.createSkill({ name: 'alpha', description: '处理甲类任务', body: 'ALPHA-BODY' });
  ...
  it('triggers 命中用户消息：agent:input:before 自动激活，该会话组装注入正文', async () => {
    const { skills, assembly, hooks } = await setup();
    ...
    await hooks.run('agent:input:before', {
    ...
    const messages = baseMessages();
    await assemblePromptContributions(assembly, { messages, sessionId: 's-wx' });
  ...
  it("技能名同形碰撞（'a/b' 归一撞 'a_b'）：守卫保先占者，后者不注入且不抛错", async () => {
    ...
    store.seed('data:/skills/slash-skill/SKILL.md', '---\nname: a/b\ndescription: 名字带斜杠\n---\n\nSLASH-BODY\n');
    store.seed(
  ...
  it('discoveryEnabled=false：discovery 与激活块都不注入', async () => {
    const { skills, assembly } = await setup({ discoveryEnabled: false });
    await skills.createSkill({ name: 'alpha', description: '甲', body: 'ALPHA-BODY' });
    expect(skills.loadSkillForSession('s-1', 'alpha')).toBe(true);
