...
async function setup(config: Record<string, unknown>) {
  ...
  await app.ctx.useModule(openaiModule as never, { apiKey: 'test-key', ...config });
...
describe('thinkingParam：request.think → DeepSeek 风格 thinking:{type}', () => {
  it('开启闸：think=true/false 各编码为 enabled/disabled，未指定则不发字段', async () => {
    const { app, llm, bodies } = await setup({ thinkingParam: true });
    await llm.chat({ messages, think: true });
    await llm.chat({ messages, think: false });
  ...
  it('开启闸：流式路径同样编码', async () => {
    const { app, llm, bodies } = await setup({ thinkingParam: true });
    for await (const _ of llm.chatStream!({ messages, think: false })) {
      // 消费完整条流
    }
    await app.stop();
  ...
  it('默认关闸（真 OpenAI 端点不认该字段）：即便请求指定 think 也不发', async () => {
    const { app, llm, bodies } = await setup({});
    await llm.chat({ messages, think: false });
    for await (const _ of llm.chatStream!({ messages, think: true })) {
      // 消费完整条流
    }
    await app.stop();
