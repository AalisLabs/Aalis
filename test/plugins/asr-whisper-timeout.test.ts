...
function makeApp(calls: Call[]): App {
  const app = new App({ config: { name: 'T', logLevel: 'error', plugins: {} } });
  app.ctx.provide('process', {
...
describe('plugin-asr-whisper-cpp: 子进程必须带超时', () => {
  ...
  it('ffmpeg 与 whisper-cli 两处 execFile 都带正数 timeout', async () => {
    ...
    expect(asr, 'asr 服务应已注册').toBeDefined();

    await asr!.transcribe({ attachment: { data: 'data:audio/wav;base64,AAAA' } });
