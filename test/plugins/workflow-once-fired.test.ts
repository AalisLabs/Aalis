...
import type { WorkflowDef, WorkflowService } from '../../packages/api-workflow/src/index.js';
...
import { TriggerManager } from '../../packages/plugin-workflow/src/triggers.js';
...
describe('workflow once 触发器只触发一次（真 fs 持久化）', () => {
  ...
  const boot = async (): Promise<{ app: App; svc: WorkflowService }> => {
    ...
    return { app, svc: app.ctx.getService<WorkflowService>('workflow')! };
  };
...
// 直接驱动 TriggerManager（once 分支不碰 ctx），配假时钟推进月级时间。
// ════════════════════════════════════════════════════════════
