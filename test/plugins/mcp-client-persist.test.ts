...
describe('mcp-client 自服务开关落盘', () => {
  it('切换 enabled：bounce 自己后运行态与配置文档一致，且落盘一次', async () => {
    ...
    const app = new App({
      config: { name: 'T', logLevel: 'error', plugins: { [NAME]: { servers: [server] } } },
      logger: silent,
      configProvider: {
        save: snapshot => {
    ...
    expect((app.config.getPluginConfig(NAME) as Servers).servers[0].enabled).toBe(true);
