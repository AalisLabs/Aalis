...
function mkConfig(cfg: Cfg = {}): ConfigManager {
  const store: Cfg = { ...cfg };
  return { get: (k: string) => store[k], set: (k: string, v: unknown) => void (store[k] = v) } as unknown as ConfigManager;
}
...
describe('authorize（deny > owner > public > granted）', () => {
  ...
  it('内置受限：写 users.json 默认禁、仅 owner/被授予', () => {
    ...
    expect(m.authorize(onebot('1'), { capability: 'tool:fw', visibility: 'public', resourceCapabilities: [cap] })).not.toBeNull();
    expect(m.authorize(onebot('boss'), { capability: 'tool:fw', visibility: 'public', resourceCapabilities: [cap] })).toBeNull();
  });
...
describe('setUserCapabilities 委托子集约束', () => {
  it('owner 可委托一切', () => {
    const m = new AuthorityManager(mkConfig({ owners: [{ platform: 'onebot', userId: 'boss' }] }), mkLogger(), storage);
    expect(() => m.setUserCapabilities(onebot('boss'), onebot('sub'), { grant: ['tool:*', 'storage:secret:write'] })).not.toThrow();
  });
  ...
  it('非 owner 只能委托自己持有的；越权抛错', () => {
    ...
    expect(() => m.setUserCapabilities(onebot('granter'), onebot('sub2'), { grant: ['storage:x:write'] })).toThrow(/越权/);
  });
