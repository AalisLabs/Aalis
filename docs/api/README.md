...
| [plugin-commands-api](./plugin-commands-api.md) | `CommandService` + `ctx.command()` —— 命令系统 | plugin-commands |
...
| [plugin-storage-api](./plugin-storage-api.md) | `StorageService` —— 受控文件/对象存储 | plugin-storage-local / plugin-storage-router |
...
4. **Context 扩展方法通过 `declare module '@aalis/core' { interface Context }`** 增强；实现包在 `apply()` 时调用 `Context.extend(...)` 注入运行时实现。
