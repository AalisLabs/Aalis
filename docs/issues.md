...
2. **授权选择器扩枚举 action:/storage:**（可选增强）：能力可视化选择器已覆盖命令/工具；`action:<plugin>:<method>`
   与 `storage:*` 仍走「高级 glob」框。如需也可视化，给 `getOverview` 补页面动作枚举 + 前端列出（storage
   路径动态，难全枚举，可保留高级框）。
...
- ✅ 2026-06-16 **授权能力可视化选择器（webui-client 0.4.2）**：高权限给低权限授权原本手写 glob
  ...
  添加用户两处。纯转换逻辑抽 capability-picker-util.ts（splitCaps/buildCaps，去重，权限安全相关）+ 7 例单测。
  委托子集越权仍由后端兜底。零后端改动。action:/storage: 可视化留待办（新功能#2）。
...
- ✅ 2026-06-16 **全仓用户可见输入面加固（batch 2，commands/cron/onebot 0.4.1）**：先研究 调研的脚手架
...
- ✅ 2026-06-16 **create-aalis 交互严格校验 + 撤终端插件大列表（0.4.2 / plugin 0.4.1）**：用户报
  「输 1,2sa / sidhu / 1, 2 都过闸不报错」+ 拥挤担忧。研究确认 调研的脚手架做法=终端只建最小项目+
  ...
  校验重问，旧放行 MyBot 等生成坏 name）；**撤掉终端 live 全列表多选**→交 WebUI 市场；
