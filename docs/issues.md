...
3. ~~扩展工具/指令的权限，参数不同可以需求不同的权限等级而不是根据当前工具/指令不变，这样可以类似存储工具的data的读是1级，但写是3级。但是否要考虑plugin等的权限如何管理？是否可能被恶意攻击者通过某种方式绕过？~~ ✅ 已修复（方案一·参数级动态提权）：原本权限等级只看工具**静态**声明的 `authority`（`file_write` 恒为 3），`3 级用户可 file_write 覆盖 data:/users.json 自我提权` 或写 data:/scheduler-jobs.json 注入 owner 身份的 actor。动态能力前半其实已存在（`resolvePermissions(args,ctx)` 已按参数产出 `storage:<root>:<op>`，guard 也已拿到 args+permissions），缺的是「敏感权限→更高等级」的映射。修法：①file 工具的 `storagePermission` 额外产出**路径级**权限标识 `storage:path:<uri>:<op>`；②authority 新增 `requiredAuthorityFor(permissions)`，内置默认保护清单（写/删 `data:/users.json`、`data:/scheduler-jobs.json` 需 owner 等级，等级随 `ownerAuthority` 配置，非硬编码数字），可由 `config.permissionAuthority`（glob→等级）覆盖/扩展；③guard 计算 `required = max(声明 authority, requiredAuthorityFor(permissions))` 再比对用户等级。新增 9 个单测锁定逻辑。**残留风险（已知，单列）**：`code_runner` 子进程用本地 `fs` 写任意路径，既不经 file 工具也不经 storage 网关，路径级提权拦不住——属进程级隔离范畴，纳入「scoped/app 沙盒」新功能计划，非本次方案能堵（方案二在数据层兜底同样拦不住该旁路，故按你的判断不做）。
