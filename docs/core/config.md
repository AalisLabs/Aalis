...
插件侧：自己的配置视图是基础服务 `config`（`uses: { config }`，这次激活的只读配置对象）；整份配置的读写是宿主服务 `hostConfig`（窄面 `HostConfig`：`get` / `getAll` / `set`、插件配置与启停、服务偏好；不含 `watch` / `save`），须显式写进 `uses`；落盘经 `appService.saveConfig()`。宿主入口是 `app.config`。
