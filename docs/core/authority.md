...
- 哪些 capability 属 restricted：内置保护清单 `BUILTIN_RESTRICTED`（写/删
  `data:/users.json`、写/删 `data:/scheduler-jobs.json`、`aalis:` 源码根的
  write/delete）＋ `config.restrictedCapabilities`（glob）叠加。
...
```typescript
authority.authorize(identity, { capability, visibility, resourceCapabilities? })
// → null 放行 | string 拒绝原因
```
...
主能力按 `request.visibility`（已应用 `visibilityOverrides`）判可见性；资源能力
（`resourceCapabilities`，如 `storage:path:...:write`）按 `restrictedCapabilities` 判受限性。
