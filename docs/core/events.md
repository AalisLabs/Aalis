...
中间件钩子是 Aalis 最强大的扩展机制。与事件不同，钩子是**有序管道**，插件可以修改管道中的数据、也可以完全中断流程。插件在 `uses` 里声明 `hooks`。
...
```typescript
// 注册中间件（同一钩子内按注册顺序执行）；随这次激活撤回
const dispose = hooks.middleware('agent:reply:before', async (data, next) => {
  data.content = processContent(data.content);
  await next();
});

// 执行管道（由 Agent 或其他插件调用）
await hooks.run('agent:reply:before', { content: '...' }, async () => {
...
- **上下文绑定**: 每个中间件关联注册方这次激活的清理归属，插件卸载时自动清理（`unregisterByOwner`）

### 典型用法

> **往提示词里加内容不要用本钩子**——那是 `agent:prompt` 贡献点的活（`contributions.contribute`，
...
await hooks.run('schedule:before', { jobId, cron }, async () => {
  // defaultAction: 执行调度任务
  await executeJob(jobId);
});

// 拦截钩子的第三方插件
hooks.middleware('schedule:before', async (data, next) => {
...
自定义 hook 需要通过 declaration merging 扩展 `HookContextMap`，这样 `hooks.middleware()` 和 `hooks.run()` 都能获得精确类型。插件须在 `uses` 里声明 `hooks`。
