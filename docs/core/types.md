...
**源码**: `packages/core/src/index.ts`（值导出定格见 `test/core/purity.test.ts`）；类型按分层散在 `types/`、`context/`、`orchestration/`。
...
interface BindingPort<P> { /* name, id, logger, closed, current, all, follow, track, registrar */ }
...
宿主普通服务：`appService`、`pluginsService`、`hostConfig`（须显式 `uses`，不是内置能力）。
