...
`agent:prompt` 贡献点的五个锚位（`identity` / `knowledge` / `context` / `turn-context` / `turn-hint`）决定了每轮发给 LLM 的消息布局：

```
[工具定义][人设卡][identity][knowledge][context] [历史 …] [turn-context][易变块][焦点指引][turn-hint][当前消息]
└────────────── 历史之前：会话级稳定 ──────────────┘ └─────────── 历史之后：按当前轮取材 ───────────┘
```

`turn-context` 的落点由组装器按三级规则确定（跨会话委派块之前，其次易变块之前，最后回退到当前 user 消息之前），保证材料落在历史结束处、当前轮诸块之前——焦点指引与平台提示仍与当前消息保持原有的邻接关系。
...
- **每轮必变、或换发言人即变的材料**（向量检索记忆、跨会话片段、发言者档案与关系、文件清单）声明 `turn-context`，落在历史之后、当前消息之前。语义上这些本就是「与当前消息相关的材料」，贴近当前消息陈述并无损失。
- **会话级稳定、偶发才变的材料**（行为准则、技能清单、会话摘要）留在历史之前的三个锚位。它们变化时（技能激活、摘要重算）缓存断一次，属于与压缩同性质的摊销成本。
