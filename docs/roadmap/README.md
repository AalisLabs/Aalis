...
| 文档 | 主题 | 状态 |
|---|---|---|
| [已知问题](./known-issues.md) | 一次大规模对抗审计的产物：多份实现、契约与调用侧脱节、复杂度账 | 待处置 |
| [Koishi 兼容层](./koishi-compat.md) | 嵌入真实 Koishi 内核让其插件直接运行 | 已验证可行，待实施 |
| [指令系统](./commands.md) | `/help` 权限过滤（待决策做不做）、`/help` 详情的 autolink 规避 | 小 |

## 阅读顺序建议

若关心**当前欠了什么债**：先看 [已知问题](./known-issues.md)。它记的是四轮对抗审计（约 50 个 agent）实测确认、当轮未修的条目，按「结构性 / 契约脱节 / 性能 / 复杂度」分组，每条附实测依据与溯源（是既有的还是新引入的）。
