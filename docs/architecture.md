...
整体停机（`app.stop()`）单飞：先冻闸并进入停机态，再 `idle()`，再发 `app:stopping`，等监听器完成后执行停机计划。监听器内再调 `stop()` 立即返回已兑现 Promise 并 warn。全部 active 插件与根激活进同一张计划：每个激活 drain 后 close。optional 依赖成环时，分量内成员先全部 drain，再任一 close。边规则见 [插件定义与能力](core/context.md)。单插件 unload / disable / bounce 只拆该插件，不享有整次停机的交接保证。动态 `services.get` 不产生依赖边，关停期间可能取到空。
