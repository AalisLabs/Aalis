...
| 钩子名 | 数据 | 用途 |
|---|---|---|
| `message:before` | `{ message, metadata }` | 修改/拦截收到的消息（图像识别、文件提取、流控拦截） |
| `message:after` | `{ message, response, sessionId, metadata }` | 处理消息回复后（摘要触发、子任务完成检测） |
| `llm-call:before` | `{ messages, tools, sessionId }` | 修改发给 LLM 的消息列表和工具（记忆注入、技能注入、工具搜索替换） |
...
| `response:before` | `{ content, sessionId }` | 修改最终回复内容（persona JSON 解析） |
