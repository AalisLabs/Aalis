...
- `AalisConfig` 仅声明基础字段（`name` / `logLevel` / `plugins` / `disabledPlugins` / `servicePreferences`）加 `[key: string]: unknown` 兜底；业务字段（owners / deniedCapabilities / visibilityOverrides 等）由对应 plugin-*-api 通过 declaration merging 注入，core 不知晓其语义
