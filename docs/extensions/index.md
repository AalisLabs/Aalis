...
| api 包 | 注入的字段 |
|---|---|
| [@aalis/plugin-authority-api](packages/plugin-authority-api/src/index.ts) | `owners` / `dangerousPolicy` / ... |

---

## 5. `Context`（Mixin 通道）

通过 `Context.prototype` 扩展为 `ctx.xxx()` 便捷方法的扩展点。
**注意**：这是 ServiceContainer 之外的第二条扩展通道，使用须谨慎（见 [docs/decisions/0001-two-pass-loading.md](docs/decisions/0001-two-pass-loading.md) 与"工程债 M2"）。

**扩展者**：

| api 包 | 注入的 Context 方法 |
|---|---|
| [@aalis/plugin-tools-api](packages/plugin-tools-api/src/index.ts) | `ctx.registerTool` / `ctx.registerToolGroup` |
| [@aalis/plugin-commands-api](packages/plugin-commands-api/src/index.ts) | `ctx.command` / `ctx.subcommand` |
