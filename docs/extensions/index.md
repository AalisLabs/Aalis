...
## 5. `Context` 领域 Helper（替代旧 Mixin 通道）

~~原 `Context.extend` Mixin 通道已在 ADR-0005 中移除。~~ 各契约包改为导出
**领域 helper**（一个普通函数，输入 `ctx`，输出 typed scoped service），调
用方在 `apply()` 内自取自用。helper 内部封装 `ctx.getService` 与 `whenService`
延迟逻辑，保留「即插即用、无需关心顺序」的体验。
...
相关：[ADR-0005](../decisions/0005-mixin-to-service-helpers.md)。
