...
**关停**：以激活为单位分 drain / close 两阶段。默认实现在 `lifecycle.onDrain` 里先 `abortInflightAndSettle()`（与 WebUI「停止生成」同语义），再等在飞回合 Promise——收尾段 memory / 钩子还在，中止后的交接写得进去；`onDispose` 再 `abortAll()` 一次，给 drain 超时或未走到收尾兜底。依赖交接放 `onDrain`：父激活若用自己子树里的服务，父 drain 先于子 close，到 `onDispose` 时子树可能已关。llm / memory / gateway 等是普通依赖，消费者整个 close 完提供者才 drain，故本插件 `onDispose` 期间那些依赖仍在。单独卸载某个提供者没有交接保证。超时沿用 core 对 `onDrain` 的 `disposeTimeoutMs`，不另加配置键（`packages/plugin-agent/src/index.ts`）。
