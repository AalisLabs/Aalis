...
完整语义（`role × kind` 正交模型、强制出口 `prepareLLMMessages`、附件 `[图片 | ref:…]` 文法、`<at id="X">` @提及约定、流式分段、`actor` 授权身份、边界与注意事项）**均在概念文档中完整阐述**：

> **→ [`docs/concepts/message-llm-pipeline.md`](../concepts/message-llm-pipeline.md)**（写 LLM provider 或写"会发消息/会读历史"插件前必读）

本页仅提供**导出符号速查表**，按文件归档；每项标注真实 `file:line`，语义细节见概念文档对应小节。
...
> 契约约束：byte-for-byte 兼容历史；写入方须保证 `desc`/`ref` 不含 `]`/`|`。不要手动拼接字符串，见概念文档 §4。
...
> 该包仅以 `import type {} from '@aalis/core'` 锚定模块身份做增强（`index.ts`），运行时无副作用。listen/emit 这些事件的方式见 `docs/core/context.md` / `docs/concepts/service-model.md`。
