...
`RegisteredTool`（`index.ts`）—— 注册项的完整形状：
...
- `useToolService(ctx): ScopedToolService`（`index.ts`）—— 推荐入口，自动填 `pluginName=ctx.id`、`register`/`registerGroup` 走 `whenService` 延迟到服务就绪。
...
// 执行：execute 永不 throw —— 失败/被拦截都以 JSON 字符串 { error } 返回（tools.ts）
