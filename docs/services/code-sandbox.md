...
[`@aalis/plugin-code-sandbox-api`](https://github.com/AalisLabs/Aalis/blob/main/packages/plugin-code-sandbox-api/src/index.ts)。
...
（[`code-sandbox-api/src/index.ts:1-9`](https://github.com/AalisLabs/Aalis/blob/main/packages/plugin-code-sandbox-api/src/index.ts) 的设计注释）。
未来可以有 `-docker` / `-wasm` / `-e2b` 等不同机制的实现，经同一 `code-sandbox` 服务名按优先级/偏好替换。

参考实现：[`@aalis/plugin-code-sandbox-os`](https://github.com/AalisLabs/Aalis/blob/main/packages/plugin-code-sandbox-os/src/index.ts)
（Linux `bubblewrap` / macOS `sandbox-exec`）。唯一消费方：
[`@aalis/plugin-tool-code-runner`](https://github.com/AalisLabs/Aalis/blob/main/packages/plugin-tool-code-runner/src/index.ts)。

---

## 1. 契约：`CodeSandboxService`

定义在 [`code-sandbox-api/src/index.ts:52-62`](https://github.com/AalisLabs/Aalis/blob/main/packages/plugin-code-sandbox-api/src/index.ts)：
...
请求体 `SandboxRunRequest`（[`index.ts:30-44`](https://github.com/AalisLabs/Aalis/blob/main/packages/plugin-code-sandbox-api/src/index.ts)）：
...
隔离策略 `SandboxPolicy`（[`index.ts:21-28`](https://github.com/AalisLabs/Aalis/blob/main/packages/plugin-code-sandbox-api/src/index.ts)）：
...
（[`plugin-process-api/src/index.ts:38-49`](https://github.com/AalisLabs/Aalis/blob/main/packages/plugin-process-api/src/index.ts)）：
...
```
取服务，未就绪/未安装实现时返回 `undefined`（[`index.ts:71-73`](https://github.com/AalisLabs/Aalis/blob/main/packages/plugin-code-sandbox-api/src/index.ts)）。

> 注意：契约包 [`package.json`](https://github.com/AalisLabs/Aalis/blob/main/packages/plugin-code-sandbox-api/package.json) 没有 `aalis.service` 块，
...
| 角色 | 包 | 入口 |
|---|---|---|
| 契约 | `@aalis/plugin-code-sandbox-api` | [`src/index.ts`](https://github.com/AalisLabs/Aalis/blob/main/packages/plugin-code-sandbox-api/src/index.ts) |
| 参考实现 | `@aalis/plugin-code-sandbox-os` | `provide('code-sandbox', …)` @ [`src/index.ts:75`](https://github.com/AalisLabs/Aalis/blob/main/packages/plugin-code-sandbox-os/src/index.ts) |
| 唯一消费方 | `@aalis/plugin-tool-code-runner` | `useCodeSandbox(ctx)` @ [`src/index.ts:174`](https://github.com/AalisLabs/Aalis/blob/main/packages/plugin-tool-code-runner/src/index.ts)；`codeSandbox.run(...)` @ [`src/runner.ts:94`](https://github.com/AalisLabs/Aalis/blob/main/packages/plugin-tool-code-runner/src/runner.ts) |

参考实现的关键点（[`code-sandbox-os/src/index.ts`](https://github.com/AalisLabs/Aalis/blob/main/packages/plugin-code-sandbox-os/src/index.ts)）：
...
- **命令改写**（[`sandbox.ts`](https://github.com/AalisLabs/Aalis/blob/main/packages/plugin-code-sandbox-os/src/sandbox.ts)，纯逻辑、便于单测）：
...
1. **`available === false` 时 `run()` 不应被调用**，但仍要防御性抛错而非裸跑——参考实现里
   `backend === 'none'` 时 `run()` 直接 throw（[`code-sandbox-os/src/index.ts:60-63`](https://github.com/AalisLabs/Aalis/blob/main/packages/plugin-code-sandbox-os/src/index.ts)）。
...
### 注册（`ctx.provide`）
参考实现在 `apply` 里探测后端、再注册单例（[`code-sandbox-os/src/index.ts:71-79`](https://github.com/AalisLabs/Aalis/blob/main/packages/plugin-code-sandbox-os/src/index.ts)）：
...
priority 取值见 [`core/src/types/service.ts:28-30`](https://github.com/AalisLabs/Aalis/blob/main/packages/core/src/types/service.ts)：
...
- 代码导出：`export const provides = ['code-sandbox']`、`export const inject = { required: ['process'] }`
  （[`code-sandbox-os/src/index.ts:17-20`](https://github.com/AalisLabs/Aalis/blob/main/packages/plugin-code-sandbox-os/src/index.ts)）。
- `package.json` 的 `aalis.service`（[`code-sandbox-os/package.json`](https://github.com/AalisLabs/Aalis/blob/main/packages/plugin-code-sandbox-os/package.json)）：
...
`code_runner` 就是这么做的（[`tool-code-runner/src/index.ts:15-19`](https://github.com/AalisLabs/Aalis/blob/main/packages/plugin-tool-code-runner/src/index.ts)）：
...
重新 `useCodeSandbox(ctx)`（[`index.ts:174`](https://github.com/AalisLabs/Aalis/blob/main/packages/plugin-tool-code-runner/src/index.ts)），
...
`code_runner` 的处理（[`runner.ts:112-132`](https://github.com/AalisLabs/Aalis/blob/main/packages/plugin-tool-code-runner/src/runner.ts)）：
...
都标了 `visibility: 'restricted'`（[`index.ts:244`、`295`](https://github.com/AalisLabs/Aalis/blob/main/packages/plugin-tool-code-runner/src/index.ts)），
受 [权限两轴](../core/authority.md) 闸门约束。

参考实现 `code-sandbox-os` **强制**（[`sandbox.ts`](https://github.com/AalisLabs/Aalis/blob/main/packages/plugin-code-sandbox-os/src/sandbox.ts)）：
...
契约在 [`code-sandbox-api/src/index.ts:18-19`](https://github.com/AalisLabs/Aalis/blob/main/packages/plugin-code-sandbox-api/src/index.ts) 明写了 v1 语义，
...
- **平台覆盖**：参考实现只覆盖 macOS（seatbelt）/ Linux（bwrap）。Windows 等其它平台 `backend='none'`，
  `code_runner` auto 模式会 fail-closed（[`code-sandbox-os/src/index.ts:37`](https://github.com/AalisLabs/Aalis/blob/main/packages/plugin-code-sandbox-os/src/index.ts)）。
- **`env` 不在 `run()` 外层重复传**：参考实现把 env 白名单注入交给 wrapper（`env -i` / `--clearenv --setenv`），
  ...
  不可信子进程清空（[`code-sandbox-os/src/index.ts:64-67`](https://github.com/AalisLabs/Aalis/blob/main/packages/plugin-code-sandbox-os/src/index.ts)）。
...
- 相关服务/插件：[`process` 契约](https://github.com/AalisLabs/Aalis/blob/main/packages/plugin-process-api/src/index.ts)（`ExecResult` / spawn 网关）·
