...
与同生态的 `@aalis/util-json-repair` 的边界（`index.ts`）：json-repair 处理「被 prompt 要求输出 JSON 的结构化数据通道」的解析问题；本库处理「自然语言对话 `content`」的渲染层问题，约定在 agent 拿到完整响应后**统一调用一次**。
...
签名：`packages/util-text-normalize/src/index.ts`

修复 **GFM 表格分隔行与表头列数不一致**的问题。

背景（`index.ts`）：`micromark-extension-gfm-table` v2 严格要求「表头列数 === 分隔行列数」，否则整个表格退化为纯文本——用户在 UI 上看到的是裸露的 `|...|...|` 字符串。LLM 经常多打或少打一个 `|`：
...
- **双向修复**（`index.ts`、`normalizeSepRow`）：分隔行列数多 → 截断；少 → 补 `---`。
- **保留对齐符号**：归一时只保留合法对齐段 `:?-+:?`（`:--` / `--:` / `:--:`），过滤掉空段（`index.ts`）。
- **跳过代码区域**：先按 ```` ``` ```` fenced block 与 inline `` ` `` 拆分，奇数索引片段原样保留（`index.ts`），避免改坏示例代码里的伪表格。
- **保守触发**：仅当上一行包含 `|` 且非空、且两侧列数都 > 0 且**确实不等**时才动手（`index.ts`）；否则原样输出。
- 空串直接返回（`index.ts`）。
...
剥离 LLM 输出 `content` 里**泄漏的「特殊 token 标记」残渣**。返回净化后的文本 + 一个 `hadLeak` 布尔（供调用方告警/遥测，`index.ts`）。

适用范围（`index.ts`）：任何「服务端用 `<…特殊字符…keyword…特殊字符…>` 形式的 special token 表达内部协议，但解析器偶发匹配失败、导致裸标记落到 `content`」的场景。当前已知典型 case 是 **DeepSeek 的 DSML**（DeepSeek Markup Language，用于表达 `tool_calls`，`index.ts`）。已知 bug 是模型会输出畸形变体（多一对全角竖线 `<｜｜DSML｜｜…>`，`｜` 为 U+FF5C），让按 `｜DSML｜` 严格匹配的服务端 parser 失败，整段裸文本留在 `content` 里（`index.ts`）。

三层剥离策略（按顺序，`index.ts`）：

1. **整段成对 block**：`<…DSML…>…</…DSML…>`，lazy 匹配到最近闭合（`blockRe`，`index.ts`）。
2. **残留单 token**：跨 chunk 边界泄漏的开/闭标签碎片（`tokenRe`，`index.ts`）。
3. **极端兜底**：未闭合的 DSML 起始片段（缺末尾 `>`，如 `<｜｜DSML｜｜tool_calls`，`partialRe`，`index.ts`）。

关键约束：

- **不反解为 tool_calls**（`index.ts`）：本函数只剥离文本，绝不尝试把 DSML 解析回 tool_call——避免触发未授权副作用，且畸形变体解析风险大。需要恢复 tool_call 是**调用方的责任**（DeepSeek 插件的做法见 §4）。
- **廉价早出 + 防 ReDoS**（`index.ts`）：先 `content.includes('DSML')`，无该字面量直接原样返回。这同时把「无 DSML 的病理输入（如海量竖线）」挡在正则外，杜绝灾难性回溯——旧实现 5000 个竖线即冻进程。正则本身改成线性（每标签内单个 `[^<>]*` + 零宽 lookahead，`index.ts`），消除回溯爆炸。
- 仅当确实发生剥离时才对结果 `trim()`（`index.ts`）；无泄漏时 `sanitized === content` 原样。
- 命名刻意**不带厂商前缀**（`index.ts`）：未来 Qwen 漏 `<|im_xxx|>`、Llama 漏 `<|python_tag|>` 等同类问题，应**扩展本函数的 regex**，而非新开 `stripQwenTokens` 等并列函数。注意：当前三条 regex 都靠 `DSML` 字面量驱动，扩展其它 token 时这些断言需要相应放宽。

### 2.3 `normalizeAssistantContent(content: string): string`

签名：`packages/util-text-normalize/src/index.ts`

**一次性应用所有净化规则**的便捷入口。顺序固定（`index.ts`）：**先 `stripLeakedSpecialTokens` 剥离结构性泄漏，再 `fixGfmTables` 修表格**。

顺序很重要：先剥掉 DSML 块再修表格，避免 DSML 标记里的竖线被表格逻辑误判为列分隔。本函数丢弃 `hadLeak`（`index.ts` 解构后只取 `sanitized`）——**需要遥测 `hadLeak` 的调用方应自己单独调 `stripLeakedSpecialTokens`**（见 §4 agent 的做法）。空串直接返回（`index.ts`）。
...
`packages/plugin-agent/src/index.ts` 导入；在组装最终回复时（`index.ts`）：先 `stripLeakedSpecialTokens` 拿 `hadLeak` 打 warn 日志（记录 session / platform / 原长 / 净化后长度），再用 `normalizeAssistantContent(content)` 产出 `finalContent`。这是「agent 拿到完整响应后统一调用一次」约定的权威落点——所有走 agent 通道的回复都已净化。
...
`packages/plugin-webui-client/src/preprocessLaTeX.ts` 导入 `fixGfmTables`，在 `preprocessLaTeX` 开头先跑一次表格归一（`preprocessLaTeX.ts`）。注释说明（`preprocessLaTeX.ts`）：agent 侧若已修复则此处等价 no-op，保留是为**兼容历史会话与未走 agent 通道的内容**。这说明同一份纯函数逻辑被「后端 provider/agent」和「前端渲染器」两端复用——正是 util 库存在的意义。
...
- **`stripLeakedSpecialTokens` 当前强绑 `DSML` 字面量**：`index.ts` 的早出和三条 regex 的 lookahead 都要求出现 `DSML`；要支持其它模型的 token 必须改源码放宽断言，而非配置。它**不解析、不恢复** tool_call（§2.2）——恢复是调用方责任。
...
- **顺序不可换**：先剥 token 再修表格（`index.ts`）；自己手动组合时务必保持此序，否则 DSML 里的全角/半角竖线可能干扰表格列数判断。
