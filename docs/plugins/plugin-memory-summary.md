...
4. 读取最近 max(`threshold`, `keepRecent`+1, 200) 条历史，把除最近 `keepRecent` 条以外的 user/assistant/tool 消息连同已有摘要交给 LLM（assistant 消息取可见正文：落库内容是结构化输出的 JSON 信封时读 metadata 的 `visibleContent`；带工具调用的 assistant 渲染为「调用 <工具名>(参数)」，tool 结果渲染为「工具结果(工具名): …」，其中工具名按 `toolCallId` 从同段历史的 assistant `toolCalls` 反查、消息自带的 `name` 只作回落，两者的参数与正文按每条 200 字硬截断），生成更新后的摘要（提示词为 `summaryPrompt`，留空则用内置提示词）
