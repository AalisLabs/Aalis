...
| 服务描述符 / `ServiceRef`                         | MCP capabilities     |
...
1. **LLM tool-calling 阶段**：agent（如 plugin-agent）调用 `tools.current?.getDefinitions({ groups })` 拼装 `tools` 参数发给模型；MCP 工具与 `file_read` / `exec` 等本地工具混在同一个数组里返给 LLM
