...
- 首次调用先请求新版 `/api/embed`：仅在明确收到 404/405（端点不存在）时判定为旧版，改用 `/api/embeddings` 并缓存该结论；网络错误、超时、5xx 视为瞬态故障，原样抛出且不记结论，下次调用重新探测。启动连通性检查就是首次调用，此时 Ollama 不可达只会留下一条告警，不会把实例钉在旧版接口上
