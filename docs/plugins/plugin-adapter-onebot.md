...
群聊场景下，非触发消息（未 @机器人 且评分不足）会被缓冲到 memory 而非立即处理。

### 图片处理

群聊中的非触发消息如果包含图片，会通过 `describeImagesForBuffering()` 调用视觉 LLM 将图片转换为文字描述后再存储。描述格式为：

```
[图片: 一张夕阳下的海滩照片，沙滩上有贝壳]
```

若视觉服务不可用或识别失败，回退为 `[图片: 识别失败]`。
