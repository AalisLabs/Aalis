  ...
  return (
    <div className={`message-group ${msg.role}`}>
      <div className="message-sender">
        ...
        {/* assistant 角标：显示本回复用的 model / 生成 token / 耗时，便于用户验证模型切换是否生效 */}
        {msg.role === 'assistant' && msg.modelInfo && (msg.modelInfo.model || msg.modelInfo.completionTokens != null) ? (
          <span
            ...
            title={[
              ...
              msg.modelInfo.completionTokens != null ? `本次生成 ${msg.modelInfo.completionTokens} tok` : null,
              msg.modelInfo.promptTokens != null ? `上下文 ${msg.modelInfo.promptTokens} tok` : null,
              msg.modelInfo.elapsedMs != null ? `耗时 ${(msg.modelInfo.elapsedMs / 1000).toFixed(1)}s` : null,
            ].filter(Boolean).join(' · ')}
          >
            {msg.modelInfo.model ?? ''}
            {msg.modelInfo.completionTokens != null ? ` · ${(msg.modelInfo.completionTokens / 1000).toFixed(1)}k` : ''}
