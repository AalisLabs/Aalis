 ...
 *   [文件: name (ID: id)]\n--- 文件内容 ---（…「--- 文件内容结束 N ---」…）\nTEXT\n--- 文件内容结束 N ---
 *                                                                           (小文件 inline，N 为随机编号)
 *   [文件: name (ID: id)]\n--- 文件内容 ---\nTEXT\n--- 文件内容结束 ---   (小文件 inline，旧格式归档)
 *   [文件: name (ID: id，SIZE KB)\n说明：...]                              (大文件引用)
 *   [文件: name (ID: id，大文件，使用 read_uploaded_file 工具读取内容)]      (大文件引用，最早的单行格式)
 *   [文件: name - 超过大小限制 (…MB > …MB)] / [文件: name - 处理失败]        (超大 / 处理失败)
 *   [文件: name (ID: id)]                                                   (降级/错误)
 *
 * 该函数将所有此类块从显示内容中移除。name 可含任意非换行字符（半角括号、方括号、「 - 」），
 * 前四步都靠名字之后固定的收尾定界，不对名字设字符限制；第五步兜底其余形态。
 */
function stripFileAttachments(content: string): string {
  // 第一步：剥离 inline 内容块。新格式从开头那行取随机编号，只认带同一编号的行首结束标记——
  // 正文里伪造的结束标记属于正文，不会让剥离提前停下、把其后的文字显示成用户原话。
  // 文件名可含任意非换行字符（「report (1).txt」「季度报告(终版).docx」），靠行尾固定的「 (ID: <hex>)]」定界。
  let result = content.replace(
    /\n?\[文件: [^\n]+? \(ID: [0-9a-f]+\)\]\n--- 文件内容 ---[^\n]*?「--- 文件内容结束 ([0-9a-f]+) ---」[^\n]*\n[\s\S]*?\n--- 文件内容结束 \1 ---/g,
    '',
  );
  // 旧格式（无编号）的归档消息：开头那行紧跟换行，与新格式互不重叠
  result = result.replace(
    /\n?\[文件: [^\n]+? \(ID: [0-9a-f]+\)\]\n--- 文件内容 ---\n[\s\S]*?--- 文件内容结束 ---/g,
    '',
  );

  // 第二步：剥离大文件引用块：首行以「 (ID: <hex>，<SIZE> KB)」收尾，次行「说明：…]」
  result = result.replace(/\n?\[文件: [^\n]+? \(ID: [0-9a-f]+，[0-9.]+ KB\)\n说明：[^\n]*\]/g, '');

  // 第三步：剥离超大 / 处理失败的简短引用
  result = result.replace(/\n?\[文件: [^\n]+? - (?:超过大小限制 \([0-9.]+MB > [0-9.]+MB\)|处理失败)\]/g, '');

  // 第四步：剥离降级引用与最早的单行大文件引用（最早的版本里 ID 形如 file_xxx_1，不是 hex）
  result = result.replace(/\n?\[文件: [^\n]+? \(ID: \w+(?:，大文件，使用 read_uploaded_file 工具读取内容)?\)\]/g, '');

  // 第五步：兜底剥离其余形态的简单文件引用（名字不含「]」）
  result = result.replace(/\n?\[文件: ([^\]]+?)\s*(?:\(ID:[^)]+\))?\]/g, '');
...
export function buildChatMessages(raw: RawMessage[]): ChatMessage[] {
  ...
  while (i < raw.length) {
    ...
    if (msg.role === 'system') {
      ...
      // webui-client 是独立浏览器 bundle，刻意不依赖后端 @aalis/schema-message。
