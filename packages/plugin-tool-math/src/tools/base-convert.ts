  ...
  ctx.registerTool({
    definition: {
      ...
      function: {
        ...
        description: '数制转换与位运算工具。支持: convert(任意进制转换 2-36)、to_all(一次输出二/八/十/十六进制)、bitwise(位运算 AND/OR/XOR/NOT/SHL/SHR)、float_analyze(IEEE 754 浮点数分析)',
        parameters: {
          type: 'object',
          properties: {
            operation: { type: 'string', description: '操作类型', enum: ['convert', 'to_all', 'bitwise', 'float_analyze'] },
            ...
            // 位运算
            bitwiseOp: { type: 'string', description: '位运算: AND, OR, XOR, NOT, SHL, SHR', enum: ['AND', 'OR', 'XOR', 'NOT', 'SHL', 'SHR'] },
      ...
      try {
        ...
        switch (op) {
          case 'convert': {
            ...
            if (isNaN(decimal)) return JSON.stringify({ error: `无效的 ${fromBase} 进制数: ${value}` });
          ...
          case 'bitwise': {
            ...
            if (isNaN(a)) return JSON.stringify({ error: '无效的数值 a' });

            if (bitwiseOp === 'NOT') {
              const result = ~a;
              return JSON.stringify({
                operation: `NOT ${a}`, result,
                binary: `~${(a >>> 0).toString(2)} = ${(result >>> 0).toString(2)}`,
              });
            }

            const b = parseInt(String(args.value2 ?? '0'), Number(args.fromBase ?? 10));
            if (isNaN(b)) return JSON.stringify({ error: '无效的数值 b' });

            let result: number;
            let symbol: string;
            switch (bitwiseOp) {
              case 'AND': result = a & b; symbol = '&'; break;
              case 'OR': result = a | b; symbol = '|'; break;
              case 'XOR': result = a ^ b; symbol = '^'; break;
              case 'SHL': result = a << b; symbol = '<<'; break;
              case 'SHR': result = a >> b; symbol = '>>'; break;
              default: return JSON.stringify({ error: `未知位运算: ${bitwiseOp}` });
            }
            return JSON.stringify({
          ...
          case 'float_analyze': {
            ...
            if (isNaN(value)) return JSON.stringify({ error: '无效的浮点数' });
            ...
            return JSON.stringify({
              value, sign: sign === '0' ? '+' : '-',
              exponentBits: exponent, exponentValue: expValue,
