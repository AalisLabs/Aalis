...
export interface MemoryService {
  ...
  // 结构化元数据存储
  //
  // 四个方法**必填**，不设可选（`?`）：三家后端从来都是全实现，可选性只产生成本——
  ...
  //    实测八个消费方里七个只需要 KV；唯一被撑爆的是 `user-relation` 的关系图，而它在自己
  //    一侧解决：一次 `loadAll` 得到的全图快照在整条处理链上传递，不重复取。
  //    （这一层也不做进程内快照缓存：不变量靠不住。）
  //    查询一旦进契约，第三方后端的门槛就从「实现几个 KV 方法」变成
  //    「实现一个查询引擎」——那会废掉 memory 多实现（inmemory/sqlite/mongodb）这件事本身。
  //
  // 2. **不加 key 前缀扫描。** 实测：11 个 `listMetadata` 调用点里只有一个低频清理路径要
  //    子集，其余都要全量（`loadAll` 本来就要全部四类，加了前缀反而要发四次查询）。
  //
