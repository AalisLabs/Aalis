...
export const DEFAULT_VISION_PROMPT =
  '请简洁、客观地描述这张图片的内容，覆盖以下要点（按需取舍，不必逐项列出）：' +
  '1) 主体对象与画面构图；' +
  '2) 主要颜色、光照、风格氛围；' +
  '3) 场景/环境（室内外、地点线索）；' +
  '4) 出现的人物或角色及其动作、表情、姿态；' +
  '5) 画面中的文字、表情包文案与含义；' +
  '6) 其他显著细节（图标、品牌、特效等）。' +
  '用中文回答，控制在 200 字以内，不要加 markdown 标记。';

export const DEFAULT_VISION_BATCH_PROMPT =
  '以下是一组（按时间或上下文顺序排列的）图片。请综合所有图片做一段统一描述，' +
  '重点说明：' +
  '1) 动态变化与时序关系（哪几张图发生了什么变化、是否是同一场景的连拍/抽帧）；' +
  '2) 各张图的主体对象、颜色、光照与场景；' +
  '3) 出现的人物/角色及其动作、表情、互动；' +
  '4) 画面中的文字/表情包文案与含义；' +
  '5) 整体想表达的事件、情绪或梗。' +
  '用中文回答，控制在 250 字以内，不要加 markdown 标记。';
