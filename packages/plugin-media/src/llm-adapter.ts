...
/**
 * Vision 详细描述 prompt：用于试卷/文档/代码/表格/PPT 等"信息密度高"图片。
 * 不限字数，要求逐项列出全部可识别内容，公式用 LaTeX。
 * 选用条件：用户/agent 显式 detail_level='detailed'，或两阶段分类判定为 document/exam/code/table 类。
 */
export const DEFAULT_VISION_DETAILED_PROMPT =
  '请用中文详细描述这张图片中的全部可识别内容，不限字数，确保信息完整：\n' +
  '1) 若是数学题/物理题/化学题/考试卷：逐题列出题号、题干、所有选项；数学公式必须用 LaTeX 包在 $...$ 或 $$...$$ 里' +
  '（例如 $\\int_0^1 x^2 dx$、$\\frac{a}{b}$、向量/矩阵/积分/求和/极限/微分方程都要用标准 LaTeX）。\n' +
  '2) 若是代码截图：逐行抄录代码，保留缩进与语法；用 Markdown 代码块包裹并标注语言。\n' +
  '3) 若是表格：用 Markdown 表格语法逐行抄录，含表头和所有单元格。\n' +
  '4) 若是文档/PPT/书页：按视觉层级列出标题、要点、段落要旨，长段可摘要但不要遗漏关键数字。\n' +
  '5) 若图中含手写笔迹/批注/箭头标记：单独列出并说明位置。\n' +
  '6) 最后用一句话点明图片类型与发送者可能的意图（求解、求识别、求解释、单纯分享等）。\n' +
  '宁可多说不要漏说。';

/**
 * 两阶段轻量分类 prompt：要求模型从 3 个标签里选一个，让客户端据此挑详细/简洁 prompt。
 * 设计目标：极短输出、易解析、覆盖率高。fallback 一律按 detailed 处理。
 */
export const VISION_CLASSIFY_PROMPT =
  '只用一个英文标签回答这张图片属于哪类，**只输出标签本身**，不要任何解释或标点：\n' +
  '- `document`：试卷、考题、文档、PPT、书页、代码截图、表格、含密集文字或公式的图\n' +
