...
//   1. 普通依赖（别的插件、兄弟模块）：消费者整个关完，提供者才开始收尾（close → providerDrain）。
//   2. 用的是自己子树里的服务：自己的收尾先于子的撤回（drain → childClose）；到自己的撤回与清理时
//      它已按归属关闭。父 drain 时整棵子树仍活着。
//   3. 用的是祖先提供的服务：不往排序图加边。归属树已保证子的撤回先于祖先的撤回，收尾在撤回之前，
//      故子 drain 时祖先仍活着。祖先若同时用这棵子树（第 2 种），第 2 种边把祖先 drain 插在子 close
//      之前，两笔收尾都能 require 到对方。不再把两边都收成 drain→drain——同一对会变成二元环，
//      Kahn 让步后父 drain 时子已死。
