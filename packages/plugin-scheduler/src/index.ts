    ...
    ctx.registerTool({
      ...
      definition: {
        ...
        function: {
          ...
          description: '创建一个新的定时/周期性自主行动计划。可以用来安排自己将来要做的事情。',
          parameters: {
            type: 'object',
            properties: {
      ...
      handler: async (args) => {
        const job: SchedulerJobConfig = {
          ...
          platform: (args.platform as string) ?? 'internal',
        ...
        service.addJob(job);
        return JSON.stringify({ ok: true, message: `任务 "${job.name}" 已创建` });
      },
    });
