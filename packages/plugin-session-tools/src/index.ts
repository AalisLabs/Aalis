import type { Context, ConfigSchema, ToolCallContext, IncomingMessage, MemoryService, Message } from '@aalis/core';
...
function resolveConfig(raw: Record<string, unknown>): PluginConfig {
  const historyRaw = (raw.historyAccess as Record<string, unknown> | undefined) ?? {};
  const scopeRaw = historyRaw.scope;
  ...
  return {
    ...
    historyAccess: {
      ...
      scope,
      includeArchivedDefault: historyRaw.includeArchivedDefault === true,
    },
  };
}
...
export function apply(ctx: Context, config: Record<string, unknown>): void {
  ...
  if (cfg.historyAccess.enabled) {
    const historyService = createSessionHistoryService(ctx, cfg.historyAccess);
    ctx.provide('session-history', historyService, { label: '会话历史读取' });
    registerSessionHistoryTools(ctx, historyService, cfg.historyAccess);
  }
...
function createSessionHistoryService(ctx: Context, cfg: PluginConfig['historyAccess']): SessionHistoryService {
  return {
